// elev.cpp
// contains class data and member functions definitions

#include "elev.h"                  // include class declarations
//////////////////////////////////////////////////////////////////////////////////
//                      function definitions for class buildings
//////////////////////////////////////////////////////////////////////////////////
building::building()                // constructor
{
    ...
    {
        car_list[k] = new elevator(this, NUM_CARS);     // this pointer send the location of object building to an elevator class
        num_cars++;
    }

    for (int j = 0; j < NUM_FLOORS; j++)        // for each floor
    {
        set_cursor_pos(3, NUM_FLOORS-j);        // put floor number on screen     // floor starts from zero
        itoa(j+1, ustring, 10);                 
        cout << setw(3) << ustring;
...
void building::show_floor_reqs() const                // display floor requests
{
    ...
    {
        set_cursor_pos(SPACING, NUM_FLOORS-j);               // confusion                      // look for the meaning of spacing
        if (floor_request[UP][j] == true)
            cout << '\x1E';                         // up arrow     // use unicode "\u2191" if it does not work
        else
            cout << ' ';
...
elevator::elevator(building* ptrB, int nc): ptrBuilding(ptrB), car_number(nc)
{
    ...
    for (int j = 0; j < NUM_FLOORS; j++)            
        destination[j] = false;                       // occupants have not pushed any buttons yet
...
//-----------------------------------------------------------------------------------
void elevator::car_display()                          // diplay elevator image
{
    ...
    switch(loading_timer)
    {
        ...
        case 1:
            ...
            break;                                    // no happy face
        case 0:
            cout << "\xDB\xDB\xDB";                   // closed foor, no happy face (default)
            break;
    }
    /////
    ...
    switch(unloading_timer)
    {
        ...
        case 1:
            cout << "\xDB\xDB\xDB";                     // closed door, no happy face
            break;
        case 0:
            cout << "\xDB\xDB\xDB";                     // closed door, no happy face
            break;
...
//-------------------------------------------------------------------------------------
void elevator::dests_display() const                    // display destinations selected by buttons inside the car
{
    for (int i = 0; i < NUM_FLOORS; i++)                    
    {
        set_cursor_pos(SPACING-2+(car_number+1)*SPACING, NUM_FLOORS-i);
...
void elevator::decide()                                 // decide what to do
{
    ...
    // flags indicate if there is another car, going in the same direction, between us and the nearest floor request (FR)
    bool car_between_up, car_between_dn;
    
    // flags indicate if there is another car, going in the opposite direction, on the opposite side of the nearest FR
    bool car_opposite_up, car_opposite_dn;

    // floor and destination of other car (not us)
    int ofloor;
    direction odir;
    ...
    // if there's an UP floor request on this floor, and if we're going up or stopped, load passengers
    ...
    // if there's a download floor request on this floor, and if we're going down or stopped, load passengers
    ...
    // check if there are other destinations or requests    // record distance to nearest request
    ...
    {                                                       // check floors above
        ...
        {
            requests_above = true;                              // if requests set flag

            if (!nearest_higher_req)
                nearest_higher_req = j;                         // if not set before, set nearest req
        }
    }
    ...
    for (j = current_floor-1; j >= 0; j--)                  // check floors below
    {
        ...
        if (ptrBuilding->get_floor_req(UP, j) || ptrBuilding->get_floor_req(DN, j))
        {
            requests_below = true;                              // if requests set flag 

            if (!nearest_lower_req)
                nearest_lower_req = j;                          // if not set befire, set nearest req      
        }     
    }
    ...
    //if destinations and we're stopped or already doing the right way, go toward destinations
    ...
    // find out if there are other cars
    // a. going in the same direction, between us and the nearest floor request or
    // b. going in the opposite direction, on the other side of the floor request
    ...
    {
        if (j != car_number)                            // if it's not us, get it's floor and direction
        {
            ...
            if ((odir == UP || odir == STOP) && requests_above)
                // if it's above us and below the nearest request  // or on same floor as us but is lower car number
            ...
            if ((odir == DN || odir == STOP) && requests_below)
                // if it's below us and above the nearest request // or on same floor as us but is lower car number
    ...
    // and there are no other cars going down between us and the FR,
    ...
    // if we're going down or stopped, and there is an FR below us,
    // and there are no other cars going down between us and the FR,
    // or below the FR going up and closer than we are,
    // then go down
...
void elevator::get_destinations()           // stop, get destinations
{
    ...
    for (int j = 0; j < NUM_FLOORS; j++)        // get floor requests
    {                                   
        set_cursor_pos(1, 24);
        cout << "Destination " << j << ": ";
